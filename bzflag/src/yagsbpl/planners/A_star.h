...
/* once inserted into bzflag, replace the printf instances
	...
	while ( !heap->empty() )
	{
		    ...
		    if(ProgressShowInterval>0) {
			    if (expandcount % ProgressShowInterval == 0)
			    {
				    ...
				    sprintf(buffer, "Number of states expanded: %d. Heap size: %d. Time elapsed: %f s.\n", 
		...
		if ( GraphDescriptor->_stopSearch( thisGraphNode->n ) )
		{
			bookmarkGraphNodes.push_back(thisGraphNode);
			    ...
			    if (ProgressShowInterval>0) {
				    if (timediff>=0.0)
					    timediff = ((float)(clock()-startclock)) / ((float)CLOCKS_PER_SEC);
				    sprintf(buffer, "Stopping search!! Number of states expanded: %d. Heap size: %d. Time elapsed: %f s.\n", 
						    expandcount, heap->size(), ((timediff>=0.0) ? timediff : difftime(time(NULL),startsecond)) );
					controlPanel->addMessage(buffer);
			    }
			#endif
		...
		if ( GraphDescriptor->_storePath( thisGraphNode->n ) )
		{
			bookmarkGraphNodes.push_back(thisGraphNode);
				...
				if (timediff>=0.0)
					timediff = ((float)(clock()-startclock)) / ((float)CLOCKS_PER_SEC);
				sprintf(buffer, "Stored a path!! Number of states expanded: %d. Heap size: %d. Time elapsed: %f s.\n", 
						expandcount, heap->size(), ((timediff>=0.0) ? timediff : difftime(time(NULL),startsecond)) );
				controlPanel->addMessage(buffer);
			#endif
		}
		...
		myfile << "Writing this to a file." << heap->size() << " " << thisGraphNode->n.x << " " << thisGraphNode->n.y << "\n"; 
		myfile.close();
		...
		for (a=0; a<thisGraphNode->successors.size(); a++)
		{
			...
			// Neighbour that are not accessible or in closed list are to be skipped  //I could cheat by just making the isAccessible thing only accept every 10th point
	    ...
	    if (ProgressShowInterval>0 && heap->empty()) {
	        sprintf(buffer, "Stopping search!! Heap is empty. Number of states expanded: %d. Heap size: %d. Time elapsed: %f s.\n", 
					   expandcount, heap->size(), ((timediff>=0.0) ? timediff : difftime(time(NULL),startsecond)) );
			controlPanel->addMessage(buffer);
		}
	#endif
